...
#define PIN    4  // Digital IO pin connected to the Neostrip.
#define NUMPIXELS      2
...
void udpLoop(void)
{
  ...
  if(millis()-t>=20){
    ...
    //FUNCSWでディープスリープ
    if(digitalRead(WIO_FUNC_PIN)==LOW){
      //ディープスリープ!!
      colorWipe(strip.Color(0, 0, 0), 5);
      for(;;){
        ESP.deepSleep(4294967294,RF_DEFAULT);
      }
    }
...
void setup() {
  ...
  pinMode(WIO_FUNC_PIN ,INPUT_PULLUP);  //これONでdeepSleep
  delay(20);

 int t=0;  
  while(digitalRead(WIO_FUNC_PIN)==HIGH){
      delay(1);
      t++;  
      if(t>2000){
      //ディープスリープ!!
  ...
  delay(10);
  while(digitalRead(WIO_FUNC_PIN)==LOW){
      delay(1);
  }
